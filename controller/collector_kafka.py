...
class IntCollector():
    ...
    def __init__(self, influx, period):
        ...
        self.kafka_topic = random_pot_topic-col
    ...
    def __send_reports(self):
        ...
        if self.kafka_producer:
                self.kafka_producer.produce(self.kafka_topic, str(json_report).encode('utf-8'))
        if json_body:
            try:
...
def start_udp_server(args):
    ...
    collector.set_kafka_producer(kafka_producer)
...
def parse_params():
    ...
    parser.add_argument("--kafka_topic", default="int_reports",
                        help="Kafka topic to send INT reports")
